...
class SellerProfileRepository {
  ...
  Future<List<SellerReviewModel>> getReviews(int userId, {int page = 1}) async {
    try {
      ...
      return list
          .map((e) => SellerReviewModel.fromJson(e as Map<String, dynamic>))
          .toList();
    } on DioException catch (e) {
      throw ApiException(
        message:
            e.response?.data?['message'] as String? ??
            'فشل في تحميل تقييمات البائع',
        statusCode: e.response?.statusCode,
      );
    }
  }
...
final sellerProfileProvider = FutureProvider.family<SellerProfileModel, int>((
  ref,
  userId,
) {
