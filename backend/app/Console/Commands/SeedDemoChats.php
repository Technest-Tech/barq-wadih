    ...
    private function writeConversation(
        ...
        $this->firestore($token, 'PATCH', "{$base}/conversations/{$convId}", [
            'fields' => [
                ...
                // The app reads peerNames/peerAvatars while the web client reads
                // participantNames/participantAvatars — seed both spellings so a
                // demo thread shows the peer's name on either platform.
                ...
                'participantNames'    => ['mapValue' => ['fields' => [
                    $demoId   => ['stringValue' => (string) $demo->name],
                    $sellerId => ['stringValue' => (string) $seller->name],
                ]]],
                'participantAvatars'  => ['mapValue' => ['fields' => [
                    $demoId   => $demo->avatar_url
                        ? ['stringValue' => $demo->avatar_url]
                        : ['nullValue' => null],
                    $sellerId => $seller->avatar_url
