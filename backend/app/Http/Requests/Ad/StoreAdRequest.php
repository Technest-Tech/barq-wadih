...
class StoreAdRequest extends FormRequest
{
    ...
    /** Derive the seller type from the authenticated account, not client input. */
    protected function prepareForValidation(): void
    {
        $this->merge([
            'seller_type' => $this->user()?->is_dealer ? 'dealer' : 'individual',
        ]);
    }
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'title' => ['required', 'string', 'min:3', 'max:100', new AcceptableContent],
            'description' => ['required', 'string', 'min:10', 'max:5000', new AcceptableContent],
            // Price is required for "fixed" and "على السوم" (negotiable); only
            // "عند الاتصال" (price_hidden) may omit it. "على السوم" also takes
            // 0 — it means "no asking price, make me an offer" — which
            // AdService stores as null. A fixed price still has to be > 0;
            // a giveaway is is_free, not a price of zero.
            'price' => [
                'nullable',
                'required_unless:price_hidden,1',
                'numeric',
                $this->boolean('is_negotiable') ? 'min:0' : 'min:1',
                'max:9999999',
            ],
            ...
            // iPhone cameras commonly return HEIC/HEIF. The production host's
            // Imagick supports both and ImageService converts them to WebP/JPEG.
    ...
    public function messages(): array
    {
        return [
            ...
            'price.min' => $this->boolean('is_negotiable')
