...
class ChatController extends BaseController
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if ($buyer->cannotInteractWith($ad->user)) {
            return $this->errorResponse(
    ...
    public function notify(Request $request, string $id): JsonResponse
    {
        $request->validate([
            ...
            'message_preview' => ['required', 'string', new AcceptableContent],
        ]);
        ...
        if ($sender->cannotInteractWith($receiver)) {
            return $this->errorResponse('لا يمكن إرسال رسائل أو إشعارات لهذا المستخدم.', 403);
        }
