...
Route::get('app-updates', \App\Http\Controllers\Api\V1\AppUpdateController::class)
    ->middleware('throttle:60,1')->name('app-updates');
Route::post('tracking/events', MarketingTrackingController::class)
...
// The preview depends on the authenticated account's individual/dealer tier.
Route::get('ads/commission-preview', [AdController::class, 'commissionPreview'])
    ->middleware('auth:sanctum')
    ->name('ads.commission-preview');
...
// Keep this static path before the dynamic users/{user} route.
Route::get('users/blocks', [UserSafetyController::class, 'blockedUsers'])
    ->middleware('auth:sanctum')
    ->name('users.blocks');
...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::post('ads/{ad}/renew', [AdController::class, 'renew'])->name('ads.renew');
    ...
    Route::prefix('chat')->name('chat.')->group(function () {
        ...
        Route::post('conversations/{id}/media', [ChatMediaController::class, 'store'])
            ->middleware('throttle:30,1')
            ->name('conversations.media.store');
    ...
    // User-generated-content safety controls required by App Review 1.2.
    Route::get('users/{user}/safety', [UserSafetyController::class, 'status'])->name('users.safety');
    Route::post('users/{user}/block', [UserSafetyController::class, 'block'])->name('users.block');
    Route::delete('users/{user}/block', [UserSafetyController::class, 'unblock'])->name('users.unblock');
    Route::post('users/{user}/report', [UserSafetyController::class, 'report'])->name('users.report');
    Route::post('safety/check-content', [UserSafetyController::class, 'checkContent'])->name('safety.check-content');

    // Sprint 10: Notifications
    Route::get('notifications', [NotificationController::class, 'index'])->name('notifications.index');
    Route::post('notifications/{notification}/read', [NotificationController::class, 'markRead'])->name('notifications.read');
    Route::post('notifications/read-all', [NotificationController::class, 'markAllRead'])->name('notifications.readAll');
    Route::post('notifications/read-by', [NotificationController::class, 'markReadBy'])->name('notifications.readBy');
