...
/**
 * The interactive seller profile. Rendered by both public routes — /@{handle}
 * and /users/{id} — which resolve the seller on the server (for metadata) and
 * hand the id down here.
 *
 * `initialProfile` is the server-fetched profile: passing it through avoids a
 * loading flash on first paint, and it is refreshed client-side so uploads and
 * bio edits still show immediately.
 */
export default function SellerProfileClient({
  ...
  useEffect(() => {
    let cancelled = false;
    // Only fall back to the skeleton when the server didn't already render
    // this seller — otherwise the profile would flash on every hydration.
    if (initialProfile?.id !== userId) setLoading(true);
              ...
              <div className={styles.actionRow}>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    // Share the canonical @handle link rather than whatever
                    // URL this page happened to be opened with.
                    const url = profile.profile_url ?? window.location.href;
