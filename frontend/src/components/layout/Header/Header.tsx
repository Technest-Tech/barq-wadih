...
export default function Header() {
  ...
  const [popularAds, setPopularAds] = useState<AdListItem[]>([]);
  const [popularLoading, setPopularLoading] = useState(false);
  const [authOpen, setAuthOpen] = useState(false);
  const [history, setHistory] = useState<string[]>([]);

  const searchTimer = useRef<NodeJS.Timeout>(null);
  const suggestTimer = useRef<NodeJS.Timeout>(null);
    ...
    try {
      ...
      if (raw) {
        ...
        if (Array.isArray(parsed))
          setHistory(parsed.filter((s) => typeof s === 'string').slice(0, HISTORY_MAX));
      }
    } catch {
      /* ignore */
    }
  ...
  // live search — 150 ms debounce for snappy feel
  useEffect(() => {
    ...
    searchTimer.current = setTimeout(async () => {
      try {
        ...
        // Only auto-open when user is actively typing, not on URL hydration.
  ...
  // autocomplete suggestions — faster, separate 100ms debounce
  useEffect(() => {
    ...
    suggestTimer.current = setTimeout(async () => {
      try {
        const data = await fetchSearchSuggestions(search.trim());
        setSuggestions(data);
      } catch {
        /* ignore */
      }
  ...
  // popular-ads fallback — fires when full search finishes with zero results
  useEffect(() => {
    if (searchLoading || searchResults.length > 0 || !search.trim()) {
      setPopularAds([]);
      return;
    }
  ...
  }, [searchLoading, searchResults.length, search]);
  ...
  const handleSearchKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      // +1 slot for the "View all" button
      const slotCount = searchResults.length + (searchResults.length > 0 ? 1 : 0);
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        setFocusedIdx((i) => Math.min(i + 1, slotCount - 1));
        ...
        e.preventDefault();
        const ad = searchResults[focusedIdx];
        pushHistory(search);
        setSearchOpen(false);
        setFocusedIdx(-1);
        router.push(`/${locale}/ads/${ad.id}`);
      }
    },
    [searchOpen, search, searchResults, focusedIdx, pushHistory, locale, router]
  );

  // bold the matched portion of a result title
  const highlightMatch = (text: string, query: string) => {
    ...
    return (
      <>
        {text.slice(0, idx)}
        <mark className={styles.searchMatchHighlight}>{text.slice(idx, idx + q.length)}</mark>
        ...
        <div className={styles.inner}>
                ...
                {search ? (
                  <>
                    ...
                    {suggestions.length > 0 && (
                      <div className={styles.searchSuggestSection}>
                        {suggestions.map((sug) => (
                          <button
                            key={sug}
                            className={styles.searchSuggestItem}
                            onClick={() => runSearch(sug)}
                          >
                            <span className={styles.searchSuggestIcon}>🔍</span>
                            <span className={styles.searchSuggestText}>
                              {highlightMatch(sug, search)}
                            </span>
                            <span className={styles.searchSuggestArrow}>↗</span>
                          </button>
                        ))}
                      </div>
                    )}

                    {/* ── Zero-results state + popular fallback ── */}
                    {!searchLoading && searchResults.length === 0 && (
                      <>
                        <div className={styles.searchEmpty}>
                          <div className={styles.searchEmptyIcon}>🔍</div>
                          <div>لا توجد نتائج لـ «{search}»</div>
                          <div className={styles.searchEmptyHint}>
                            جرّب كلمات مختلفة أو تصفّح الأقسام
                          </div>
                        </div>
                        {(popularLoading || popularAds.length > 0) && (
                          <div className={styles.searchPopularSection}>
                            <div className={styles.searchPopularHeader}>⭐ ربما يعجبك</div>
                            {popularLoading ? (
                              <div className={styles.searchLoading}>
                              ...
                              popularAds.map((ad) => (
                                ...
                                >
                                  <div className={styles.searchResultImg}>
                                    {ad.primary_image ? (
                                      <img src={ad.primary_image.thumbnail_url} alt="" />
                                    ) : (
                                      '📷'
                                    )}
                                  </div>
                                  <div className={styles.searchResultInfo}>
                                    ...
                                    <div className={styles.searchResultMeta}>
                                      {ad.category && (
                                        <span className={styles.searchResultCategoryBadge}>
                                          {ad.category.name_ar}
                                        </span>
                                      )}
                    ...
                    {!searchLoading &&
                      searchResults.map((ad, idx) => (
                        <Link
                          key={ad.id}
                          href={`/${locale}/ads/${ad.id}`}
                          className={`${styles.searchResult} ${focusedIdx === idx ? styles.searchResultFocused : ''}`}
                          onClick={() => {
                            pushHistory(search);
                            setSearchOpen(false);
                            setFocusedIdx(-1);
                        ...
                        >
                          ...
                          <div className={styles.searchResultInfo}>
                            <div className={styles.searchResultTitle}>
                              {highlightMatch(ad.title, search)}
                            </div>
                            <div className={styles.searchResultMeta}>
                              {ad.category && (
                                <span className={styles.searchResultCategoryBadge}>
                          ...
                          <div className={styles.searchResultPrice}>
                            {ad.is_free ? (
                              ...
                              <strong>{Number(ad.price).toLocaleString('ar-SA')} ر.س</strong>
                            ) : (
                              <span>على السوم</span>
                            )}
                          </div>
                    ...
                    {!searchLoading && searchResults.length > 0 && (
                      <button
                        type="button"
                        className={`${styles.searchViewAll} ${focusedIdx === searchResults.length ? styles.searchViewAllFocused : ''}`}
                        onMouseEnter={() => setFocusedIdx(searchResults.length)}
                        onClick={() => runSearch(search)}
                      >
                        عرض كل النتائج{' '}
                        {searchTotal > 0 && `(${searchTotal.toLocaleString('ar-SA')})`} ←
                      </button>
                    )}
                  </>
                ) : (
                    ...
                    {history.length > 0 && (
                      <>
                        <div className={styles.searchDefHeader}>
                          <button
                            type="button"
                            className={styles.searchClearHist}
          ...
          <div className={styles.actions}>
            {isAuthenticated ? (
                ...
                <div className={styles.userMenu} ref={userDropRef}>
                  ...
                  {userDropOpen && (
                    <div className={styles.userDropdown}>
                      ...
                      <div className={styles.userDropDivider} />
                      <button
                        className={`${styles.userDropItem} ${styles.userDropLogout}`}
              ...
              <>
                <button onClick={() => setAuthOpen(true)} className={styles.loginBtn}>
                  تسجيل الدخول
                </button>
      ...
      >
        ...
        {isAuthenticated && (
          <nav className={styles.drawerNav}>
            ...
            <button
              className={`${styles.drawerNavItem} ${styles.drawerLogout}`}
              onClick={handleLogout}
            >
        ...
        <div className={styles.drawerFooter}>
          {isAuthenticated ? (
            <Link
              href={`/${locale}/post-ad`}
              className={styles.drawerPostBtn}
              onClick={() => setMenuOpen(false)}
            >
              <Plus size={18} />
