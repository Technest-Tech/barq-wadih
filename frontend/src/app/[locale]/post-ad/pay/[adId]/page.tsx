...
import { PaymentLauncher } from '../../_components/pay/PaymentLauncher';
import { usePostAdWizard } from '@/store/postAdWizard.store';

/**
 * Pay landing page — kicks off after the wizard creates a paid-tier ad.
 * Loads the ad to surface the publish fee, lets the user pick Apple Pay
 * or Mada (mocked), then redirects to the ad detail page on success.
 */
export default function PayPage() {
  ...
  useEffect(() => {
    ...
    fetchAd(adId).then(setAd).catch((e) => setError((e as Error).message));
  }, [ready, adId]);
    ...
    <>
      ...
      <main className={styles.main}>
        <div className={styles.shell}>
          <h1 className={styles.pageTitle}>إكمال الدفع لنشر الإعلان</h1>
            ...
            <div className={styles.card}>
              <div className={styles.reviewCard}>
                ...
                <div className={styles.reviewBody}>
                  ...
                  <div className={styles.reviewPrice}>
                    رسوم النشر: {Number(ad.payment_amount ?? 0).toLocaleString('ar-SA')} ر.س
                  </div>
                </div>
              </div>
              ...
              {ad.payment_status === 'paid' ? (
                <div style={{ marginTop: 16, padding: 14, background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.3)', borderRadius: 12 }}>
                  ✅ تم الدفع. الإعلان منشور الآن.
                  <div style={{ marginTop: 10 }}>
                    <button
                      className={`${pm.pmBtn} ${pm.pmBtnPrimary}`}
                      onClick={() => { reset(); router.push(`/ar/ads/${ad.id}`); }}
                    >
                      عرض الإعلان
                    </button>
                  </div>
