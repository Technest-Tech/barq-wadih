...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const page = await fetchStaticPage('fees');
  const isAr = locale === 'ar';
  return {
    title: isAr ? (page?.title_ar ?? 'الرسوم والأسعار') : (page?.title_en ?? 'Fees & Pricing'),
...
export default async function FeesPage({ params }: Props) {
  ...
  const title   = isAr ? (page?.title_ar   ?? 'الرسوم والأسعار') : (page?.title_en   ?? 'Fees & Pricing');
