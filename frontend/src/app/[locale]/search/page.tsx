...
export default function SearchPage() {
  ...
  const pushUrl = useCallback((filters: SearchFilters) => {
  ...
  const triggerSearch = useCallback((pg = 1) => {
        ...
        <div className={styles.layout}>
          ...
          <aside className={styles.sidebar}>
            <FilterPanel
              ...
              onRegion={v  => { setRegionId(v); setCityId(undefined); }}
            ...
            {filterOpen && (
              <div className={styles.mobileFilter}>
                <FilterPanel
                  ...
                  onPriceMax={v => setPriceMax(v)}
                  onSort={v => setSort(v)}
                  onApply={() => { triggerSearch(1); setFilterOpen(false); }}
                  onReset={() => {
                    setCategoryId(undefined);
                    setRegionId(undefined);
                    setCityId(undefined);
              ...
              <div className={styles.empty}>
                ...
                {activeFilters > 0 && (
                  <button
                    ...
                    onClick={() => {
                      setCategoryId(undefined); setRegionId(undefined);
                      setCityId(undefined); setPriceMin(''); setPriceMax('');
                      triggerSearch(1);
                    }}
                  >
                    مسح الفلاتر
    ...
    <Link href={`/ar/ads/${ad.id}`} className={styles.card}>
      <div className={styles.cardImg}>
        {ad.primary_image ? (
          ...
          <img src={ad.primary_image.thumbnail_url} alt={ad.title} loading="lazy" />
        ) : (
...
function FilterPanel({
  ...
  categoryId, regionId, cityId, priceMin, priceMax, sort,
  onCategory, onRegion, onCity, onPriceMin, onPriceMax, onSort, onApply, onReset,
}: FilterPanelProps) {
  return (
    <div className={styles.filterPanel}>
      <div className={styles.filterHeader}>
        <span className={styles.filterTitle}>🎛 الفلاتر</span>
        <button className={styles.filterReset} onClick={onReset}>مسح الكل</button>
      </div>
