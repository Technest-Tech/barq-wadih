...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const page = await fetchStaticPage('delete-account');
  const isAr = locale === 'ar';
  return {
    title: isAr ? (page?.title_ar ?? 'حذف الحساب') : (page?.title_en ?? 'Delete Account'),
...
export default async function DeleteAccountPage({ params }: Props) {
  ...
  const title   = isAr ? (page?.title_ar   ?? 'حذف الحساب') : (page?.title_en   ?? 'Delete Account');
