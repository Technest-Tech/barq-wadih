...
export default function MyAdsPage() {
  ...
  const handleSold = async (id: number) => {
    ...
    if (updated) setAds(prev => prev.map(a => a.id === id ? { ...a, status: 'sold', status_label: 'مُباع' } : a));
    setActionId(null);
  };
  ...
  const handleBoost = async () => {
    ...
    try {
      ...
      setAds(prev => prev.map(a =>
        a.id === boostTarget
          ? { ...a, is_boosted: true, boosted_until: new Date(Date.now() + 72 * 60 * 60 * 1000).toISOString() }
          : a
      ));
    } catch (err: unknown) {
      ...
      <main className={styles.main}>
        <div className={styles.container}>
          <div className={styles.topRow}>
            ...
            <Link href="/ar/post-ad" className={styles.btnNew}>+ إعلان جديد</Link>
          </div>
          ...
          {!loading && filtered.length === 0 && (
            <div className={styles.empty}>
              ...
              <h3>{activeTab === 'all' ? 'لا توجد إعلانات بعد' : `لا توجد إعلانات بحالة "${STATUS_TABS.find(t => t.key === activeTab)?.label}"`}</h3>
              {activeTab === 'all' && <p>انشر إعلانك الأول الآن وابدأ في البيع!</p>}
              {activeTab === 'all' && <Link href="/ar/post-ad" className={styles.btnNew}>نشر إعلان</Link>}
            </div>
          )}
          ...
          {!loading && filtered.length > 0 && (
            <div className={styles.adList}>
              {filtered.map(ad => {
                ...
                const daysLeft = daysUntil(ad.published_at ? (ad as AdListItem & { expires_at?: string }).expires_at ?? null : null);
                const isExpiringSoon = daysLeft !== null && daysLeft <= 5 && daysLeft > 0 && ad.status === 'active';
                  ...
                  <div key={ad.id} className={`${styles.adCard} ${busy ? styles.adCardBusy : ''}`}>
                    ...
                    <div className={styles.adBody}>
                      <div className={styles.adMeta}>
                        ...
                        <span className={styles.adDate}>{new Date(ad.created_at).toLocaleDateString('ar-SA')}</span>
                      </div>
                      <Link href={`/ar/ads/${ad.id}`} className={styles.adTitle}>{ad.title}</Link>
                      <p className={styles.adPrice}>
                        {ad.is_free ? 'مجاني' : `${Number(ad.price ?? 0).toLocaleString('ar-SA')} ر.س`}
                        {ad.is_negotiable && <span className={styles.neg}>قابل للتفاوض</span>}
                      </p>
                      <p className={styles.adLocation}>{ad.city?.name_ar} • {ad.category?.name_ar}</p>
                      {isExpiringSoon && (
                      ...
                      {ad.status === 'active' && (
                        <>
                          ...
                          {!ad.is_boosted || !hoursUntil(ad.boosted_until) ? (
                            ...
                            <div>
                              <span className={styles.boostedBadge} style={{ position: 'static', fontSize: '.72rem' }}>⚡ مميز</span>
                              <p className={styles.boostCountdown}>باقي {formatCountdown(ad.boosted_until)}</p>
                            </div>
                          )}
                          ...
                          <button className={`${styles.actionBtn} ${styles.soldBtn}`} disabled={busy} onClick={() => handleSold(ad.id)}>
                            {busy ? '...' : '✓ مُباع'}
                          </button>
                        </>
                      )}
                      {(ad.status === 'active' || ad.status === 'pending_review') && (
                        <Link href={`/ar/post-ad?edit=${ad.id}`} className={`${styles.actionBtn} ${styles.editBtn}`}>تعديل</Link>
                      )}
                      {ad.status !== 'deleted' && (
                        <button className={`${styles.actionBtn} ${styles.deleteBtn}`} disabled={busy} onClick={() => handleDelete(ad.id)}>
                          {busy ? '...' : 'حذف'}
                        </button>
                      )}
                    </div>
          ...
          <div className={styles.modalOverlay} onClick={() => setBoostTarget(null)}>
            <div className={styles.modalCard} onClick={e => e.stopPropagation()}>
              ...
              <p>
                ...
                <br />مجاني خلال الفترة التجريبية!
              </p>
              <div className={styles.modalActions}>
                <button className={styles.modalConfirm} onClick={handleBoost} disabled={boostLoading}>
