...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const page = await fetchStaticPage('privacy');
  const isAr = locale === 'ar';
  return {
    title: isAr ? (page?.title_ar ?? 'سياسة الخصوصية') : (page?.title_en ?? 'Privacy Policy'),
...
export default async function PrivacyPage({ params }: Props) {
  ...
  const title   = isAr ? (page?.title_ar   ?? 'سياسة الخصوصية') : (page?.title_en   ?? 'Privacy Policy');
