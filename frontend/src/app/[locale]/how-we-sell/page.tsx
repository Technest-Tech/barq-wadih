...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const page = await fetchStaticPage('how-we-sell');
  const isAr = locale === 'ar';
  return {
    title: isAr ? (page?.title_ar ?? 'كيف نبيع') : (page?.title_en ?? 'How to Sell'),
    description: isAr ? page?.meta_description_ar : page?.meta_description_en,
  };
}

export default async function HowWeSellPage({ params }: Props) {
  const { locale } = await params;
  const isAr = locale === 'ar';
  const page = await fetchStaticPage('how-we-sell');

  const title   = isAr ? (page?.title_ar   ?? 'كيف نبيع') : (page?.title_en   ?? 'How to Sell');
            ...
            <>
              ...
              {page?.updated_at && (
                <p className={s.updatedAt}>
                  ...
                  {new Date(page.updated_at).toLocaleDateString(isAr ? 'ar-SA' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                </p>
              )}
            </>
          ) : (
