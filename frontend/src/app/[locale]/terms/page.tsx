...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const page = await fetchStaticPage('terms');
  const isAr = locale === 'ar';
  return {
    title: isAr ? (page?.title_ar ?? 'الشروط والأحكام') : (page?.title_en ?? 'Terms & Conditions'),
...
export default async function TermsPage({ params }: Props) {
  ...
  const title    = isAr ? (page?.title_ar    ?? 'الشروط والأحكام')   : (page?.title_en    ?? 'Terms & Conditions');
