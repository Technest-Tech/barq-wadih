...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const page = await fetchStaticPage('how-we-buy');
  const isAr = locale === 'ar';
  return {
    title: isAr ? (page?.title_ar ?? 'كيف نشتري') : (page?.title_en ?? 'How to Buy'),
    description: isAr ? page?.meta_description_ar : page?.meta_description_en,
  };
}

export default async function HowWeBuyPage({ params }: Props) {
  const { locale } = await params;
  const isAr = locale === 'ar';
  const page = await fetchStaticPage('how-we-buy');

  const title   = isAr ? (page?.title_ar   ?? 'كيف نشتري') : (page?.title_en   ?? 'How to Buy');
